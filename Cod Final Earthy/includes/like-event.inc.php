<?php   
    ...
    if (isset($_POST['state']) && isset($_SESSION['userId'])) {
        ...
        if ($state == -1) {
            // See if going_state is 1 
            $sql_going = "SELECT * FROM evlikes WHERE user_id=? AND event_id=?";
            $stmt_going = mysqli_stmt_init($conn);

            if (mysqli_stmt_prepare($stmt_going, $sql_going)) {
                mysqli_stmt_bind_param($stmt_going, "ii", $user_id, $event_id);
                mysqli_stmt_execute($stmt_going);
                $result_going = mysqli_stmt_get_result($stmt_going);
                if ($row_going = mysqli_fetch_assoc($result_going)) {
                    $delete = $row_going['going_state'];
                }
                else {
                    $delete = 0;
                }
            ...
            else { // I have to update only the heart_state
                $sql_update = "UPDATE evlikes SET heart_state=0 WHERE user_id=? AND event_id=?";
            ...
            if ($exists == 1) { // I have to update
                $sql_update = "UPDATE evlikes SET heart_state=1 WHERE user_id=? AND event_id=?";
                $stmt_update = mysqli_stmt_init($conn);
                if (mysqli_stmt_prepare($stmt_update, $sql_update)) {
                    mysqli_stmt_bind_param($stmt_update, "ii", $user_id, $event_id);
                    mysqli_stmt_execute($stmt_update);
        ...
        $sql_update = "UPDATE events SET event_likes=event_likes+$state WHERE event_id=?";
        $stmt_update = mysqli_stmt_init($conn);
        if (mysqli_stmt_prepare($stmt_update, $sql_update)) {
        ...
        if (mysqli_stmt_prepare($stmt, $sql)) {
            ...
            if ($row = mysqli_fetch_assoc($result)) {
                echo number_format($row['event_likes'], 0, ".", ".");
            }
            else echo '0';
        }
        else echo '0';
